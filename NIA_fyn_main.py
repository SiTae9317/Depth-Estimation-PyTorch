...
class NIADataset(data.Dataset):
    def __init__(self, train=True):
        ...
        if train:
            self.name_map = pickle.load(open("{}/{}/index1.pkl".format(LOAD_DIR,DATASETDIR),'rb'))
            self.rgb_paths = list(self.name_map.keys())
        else:
            self.name_map = pickle.load(open("{}/{}/index2.pkl".format(LOAD_DIR,DATASETDIR),'rb'))
            self.rgb_paths = list(self.name_map.keys())
    ...
    def __getitem__(self, index):
        path = '{}/{}/{}_rgb/'.format(LOAD_DIR,DATASETDIR,self.load_type)+self.rgb_paths[index]
        rgb = Image.open(path)
        depth = Image.open('{}/{}/{}_depth/'.format(LOAD_DIR,DATASETDIR,self.load_type)+self.name_map[self.rgb_paths[index]])
...
if __name__ == '__main__':
    ...
    eval_dataset = NIADataset(train=False)
    print(train_size)
